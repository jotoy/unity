...
public class JoystickController : MonoBehaviour {
	...
	void Update () 
	{
        ...
        while (i < 4)
        {
            if (Mathf.Abs(Input.GetAxisRaw("Y axis")) > 0.2F || Mathf.Abs(Input.GetAxisRaw("X axis")) > 0.2F)
                joyName = Input.GetJoystickNames()[i] + " is moved";
	...
	void OnGUI()
	{
        ...
        GUI.TextArea(new Rect(400, 400, 250, 50), "JoyName : " + joyName);
	}
}
